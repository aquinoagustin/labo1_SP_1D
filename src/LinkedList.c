...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    int tam=ll_len(this);
    Node* auxNodo;
    Node* auxNodoAnterior;
    auxNodo =(Node*) malloc(sizeof (Node));
    if(this!=NULL&&nodeIndex<tam+1&&nodeIndex>-1)
    {
...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this!=NULL&&index>-1&&index<tam)
    {
    	...
    	{
			auxAnteriorNode=getNode(this,index-1);
			auxNode=getNode(this,index);
    		if(index>0&&index<tam)
    		{
    			auxAnteriorNode->pNextNode=auxNode->pNextNode;
    		}
    		else
    		{
    			auxAnteriorNode->pNextNode=NULL;
    		}
    		//auxNode=auxAnteriorNode->pNextNode;
    	}
    	this->size--;
    	free(auxNode);
    	auxNode=NULL;
...
int ll_push(LinkedList* this, int index, void* pElement)
{
   ...
   /* int tam=ll_len(this);
    Node* nodeDireccionNueva;
    Node* nodeActual;
    Node* nodeNuevo;
    nodeDireccionNueva=(Node*)malloc(sizeof(Node));
    if(this!=NULL &&index>-1 &&index<tam)
    {
    	nodeActual=getNode(this,index);
		nodeNuevo=nodeActual;
		nodeActual=nodeDireccionNueva;
    	if(index==0)
		{
			this->pFirstNode=nodeNuevo;
			nodeNuevo->pNextNode=nodeActual;
		}
    	else
    	{
    		if(index>0&&index<tam)
...
LinkedList* ll_filter(LinkedList* this, int (*pFunc)(void*))
{
	...
	{
		clone=ll_clone(this);
		int tam=ll_len(clone);
		if(clone!=NULL)
		{
			for(int i=0;i<tam;i++)
			{
				pElement=ll_get(clone,i);
